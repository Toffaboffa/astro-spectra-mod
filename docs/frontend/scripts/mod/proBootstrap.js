  ...
  function ensurePanelContent() {
    ...
    if (core && !core.dataset.built) {
      ...
      card.innerHTML = [
        ...
        '      <label id="spFieldZoomScroller" class="sp-field sp-field--zoom-scroller sp-field--zoom-scroller-inline" title="Drag to move within the current zoom span."><span>Zoom scroller</span><input class="form-range darker-slider w-auto" type="range" id="spZoomScrollerProxy" min="0" max="100" value="0" disabled /></label>',
        '    </div>',
...
function ensureHardwarePanel() {
  ...
  card.innerHTML = [
    ...
    '  <div class="sp-card-sub sp-hw-response">',
...
function renderConsole() {
  ...
  function renderStatus() {
    ...
    if (graphXAxisSel && !shouldSkipSyncValue(graphXAxisSel)) {
      const nm = $('toggleXLabelsNm');
      const xMode = (nm && nm.checked) ? 'nm' : 'px';
      if (graphXAxisSel.value !== xMode) graphXAxisSel.value = xMode;
    }
    if (graphPeaksInput && !shouldSkipSyncValue(graphPeaksInput)) {
      const legacyPeaks = $('togglePeaksCheckbox');
      graphPeaksInput.checked = !!(legacyPeaks && legacyPeaks.checked);
    }
