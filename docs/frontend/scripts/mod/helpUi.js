(function (global) {
  ...
  function contentsHtml() {
    ...
    '<section class="sp-help-section"><h2>Implementation status and scientific limits</h2><dl class="sp-help-dl">' +
      '<dt>IMPLEMENTED</dt><dd>Shared preprocessing, calibration diagnostics, emission/absorption feature measurement, deterministic quality, LAB analysis, ASTRO continuum/absorption analysis, numeric/PDF export, interactive peak inspection and the bundled N₂, Ne, Ar, fluorescent-tube and Solar examples.</dd>' +
  ...
  function quickHtml() {
    return '<section class="sp-help-section"><h2>Quick Start</h2><p>This is the shortest reliable workflow. The detailed control reference is in the CONTROLS tab.</p></section>' +
      '<div class="sp-help-steps">' +
        step('1', 'Select the source', 'Choose a camera from the source selector, press <b>Load Image</b>, or use <b>Load Example</b>. N₂ spectral tube, Ne spectral tube, Ar spectral tube and the Philips MASTER TL5 HE 28W/830 Fluorescent tube example apply the SPECTRA-1 calibration and automatically select the <b>KVANT - Spectra-1</b> hardware profile. N₂, Ne and Ar use Gas Tube; the fluorescent tube uses Fluorescent. The numeric Solar spectrum applies its own wavelength grid, clears sample-owned SPECTRA-1 hardware, opens ASTRO and enables analysis automatically.') +
  ...
  function workspaceHtml() {
    return '<section class="sp-help-section"><h2>Workspace</h2><p>The Recording page has four functional regions plus global language and HELP controls.</p></section>' +
      '<div class="sp-help-grid2">' +
        ...
        infoCard('2 · Spectrum graph', '<p>The upper-right graph plots the stripe signal against pixels or calibrated wavelength. The color strip above the graph is the sampled spectral image. Mouse coordinates show the current graph position. Drag across the graph to define a zoom range; when a detectable peak is clicked, Peak Inspector can lock to it and show measurement/match details.</p>') +
        ...
        infoCard('6 · EN / SV', '<p>The language buttons beside the PRO tabs switch the dynamic interface between English and Swedish. English is the initial language after page load. Results, dialogs and the Peak Inspector refresh when the language changes.</p>') +
        infoCard('7 · HELP', '<p>HELP opens this modal without changing app mode. If a live camera is playing, it is paused while HELP is open and resumes when HELP closes; a camera that was already paused stays paused. Close it with ×, Escape or by clicking outside the dialog.</p>') +
      '</div>' +
  ...
  function controlsHtml() {
    const sourceRows = [
      ...
      ['Live', 'Button', 'Returns from Pause, a loaded image or a bundled example to the selected live camera.', 'The selected camera is restarted if necessary. Sample-owned calibration is cleared when returning from an example; a manually established calibration is preserved by the camera workflow.'],
      ...
      ['Auto Pause', 'One-shot button', 'Arms an automatic pause when the live extracted spectrum first reaches a peak intensity of 238/255 or higher, targeting about 240 before clipping.', 'If the camera is not live, the control first attempts to return to Live. When triggered it pauses once, logs the captured peak, and disarms. Pressing Auto Pause again while armed disarms it.'],
      ...
      ['Load Example', 'Button', 'Opens the bundled sample chooser.', 'N₂ spectral tube and Ne spectral tube are calibrated SPECTRA-1 images; Ar spectral tube is a measured 1280-sample SPECTRA-1 RGB profile; Fluorescent tube is the calibrated warm-white Philips MASTER TL5 HE 28W/830 image; Solar spectrum is calibrated numeric TSIS-1 HSRS data from 388–670 nm. The SPECTRA-1 examples automatically select KVANT - Spectra-1 hardware. N₂/Ne/Ar recommend Gas Tube, the fluorescent tube recommends Fluorescent, and Solar switches to ASTRO and enables analysis.'],
    ...
    const coreRows = [
      ...
      ['Saturation', 'Persistent graph checkbox', 'Highlights locally saturated graph regions.', 'Use it as a visual warning that clipped samples have lost peak-shape and relative-intensity information. The overlay is diagnostic only and does not repair saturation.'],
      ['Overlays → Diffraction', 'Persistent graph checkbox', 'Shows or hides possible higher-order diffraction markers identified from coherent parent/child wavelength relationships.', 'Treat these markers as artifact candidates, not chemical identifications. Diffraction-aware LAB evidence can exclude marked higher-order candidates from positive atomic evidence.'],
      ['Overlays → Extrapolation', 'Persistent graph checkbox', 'Shades wavelengths outside the interval spanned by the active calibration anchors.', 'Extrapolated wavelength values can be drawn but are less trustworthy than interpolation between calibration points.'],
      ['Overlays → Shade', 'Slider', 'Sets the tint strength of the calibration-extrapolation regions.', 'Visual only; it changes overlay opacity, not calibration or measured intensity.'],
      ...
      ['Peak Inspector', 'Graph click', 'Click a detectable peak to lock an inspector to that feature. It reports wavelength when calibrated, pixel, intensity, FWHM/SNR when available, element match details, possible diffraction artifact information and up to three quality flags.', 'A match card is evidence from the active analysis result, not an independent identification. Click the graph again to release the locked inspector.'],
      ['Peak Inspector navigation', 'Keyboard: ← / → (also < / >)', 'While Peak Inspector is locked, moves to the previous or next available scientific feature/peak inside the visible zoom range.', 'The shortcut is ignored while typing in an input, textarea, select or editable element.'],
      ['Drag on graph', 'Graph interaction', 'Dragging horizontally across the plot defines a new zoom range.', 'A drag longer than a few pixels creates the zoom; a short click is reserved for Peak Inspector. Use Step back or Reset zoom to undo graph zoom states.'],
      ...
      ['EN / SV', 'Language buttons', 'Switches the dynamic SPECTRA PRO interface between English and Swedish.', 'English is the initial language after page load. The switch updates normal UI text, frequently refreshed analysis panes, dialogs and Peak Inspector labels without changing measurement data.'],
      ...
      ['Report (.pdf)', 'Export checkbox', 'Generates the structured PDF report locally in the browser.', 'The cover uses the bundled SPECTRA PRO hero. In the spectrum/source page, the current graph and a center-cropped source spectrum are rotated into portrait-oriented columns and placed side by side. The report includes a deterministic abstract, concise method summary, compact matched features, Quality/Status, calibration/instrument context and a bounded analysis log. A completed AI interpretation may appear only in a separate, clearly labelled optional section with a disclaimer; it does not replace the rule-generated report.'],
    ...
    const hardwareRows = [
      ...
      ['Instrument response', 'Drop-down', 'Selects the relative spectral-response profile used by the preprocessing pipeline, or None for uncorrected relative intensity.', 'No fabricated response curve is bundled. A profile is applied only when its wavelength coverage and hardware compatibility support the current calibrated frame; correction factors are bounded and remain relative, not absolute radiometry.'],
    ...
    const astroRows = [
      ['Analyze', 'Checkbox', 'Enables ASTRO continuum normalization and absorption analysis in the shared worker.', 'ASTRO requires a calibrated wavelength axis for meaningful line/reference matching. Turning it on also makes the worker available in Auto mode.'],
      ['Advanced ASTRO details', 'Button', 'Opens absorption-label settings and reference-spectrum comparison controls.', 'Use this when the default ASTRO summary is not enough; it does not change the underlying source pixels.'],
      ['Show labels', 'Checkbox', 'Shows identified absorption-line labels on the graph.', 'Only measured absorption features that also have an identified reference line are labelled.'],
      ['Minimum dip depth', 'Number input 0.01–0.95', 'Sets the minimum normalized absorption depth considered for graph labels.', 'Raise it to suppress shallow/noisy labels; lower it to show weaker identified dips. It affects label display selection, not the underlying raw spectrum.'],
      ['Minimum label spacing (px)', 'Number input 8–240', 'Sets the minimum horizontal label separation.', 'Increase it when labels overlap. It is a graph-annotation spacing control.'],
      ['Reference', 'Drop-down', 'Chooses a curated or custom numeric spectrum for reference comparison.', 'The bundled catalog includes Hydrogen (H I), Helium (He I), Neon (Ne I), Mercury (Hg I) and Solar (TSIS-1 HSRS), plus a custom numeric reference. None disables comparison.'],
      ['Normalization', 'Drop-down: Min–max / Area / None', 'Chooses how the measured and reference spectra are visually normalized before comparison.', 'Min–max compares relative shape after range scaling; Area normalizes integrated area; None keeps the supplied relative amplitudes. This is comparison normalization, not abundance calibration.'],
      ['Alignment', 'Drop-down: None / Manual / Automatic comparison', 'Controls wavelength alignment of the comparison reference.', 'Automatic comparison searches an allowed shift and reports the comparison result. It is separate from ASTRO radial velocity and must not be interpreted as a barycentric/heliocentric velocity measurement.'],
      ['Manual shift (nm)', 'Number input −20…20', 'Offsets the comparison reference by the entered wavelength shift.', 'Use for controlled visual alignment tests. It does not alter the wavelength calibration of the measured spectrum.'],
      ['Load custom JSON/CSV', 'Button + file selector', 'Loads a custom numeric reference spectrum.', 'Use wavelength/intensity numeric data. The resulting custom reference becomes selectable in the comparison control.'],
      ['Reference spectrum comparison', 'Read-only text/trace', 'Shows overlap/coverage and comparison metrics such as alignment shift, correlation and RMSE when available; the aligned reference is drawn as a purple trace.', 'Correlation/RMSE describe similarity within the overlapping compared samples. A visually good aligned trace is supporting comparison evidence, not an automatic identification or radial-velocity result.']
    ];
  ...
  function qaHtml() {
    return '<section class="sp-help-section"><h2>Q&A and troubleshooting</h2></section>' +
      ...
      qa('What exactly does Auto Pause do?', 'It is a one-shot live-camera aid. When armed, SPECTRA PRO watches the extracted live spectrum and pauses as soon as its strongest intensity reaches 238/255 or higher, with a target near 240 before clipping. It then disarms. It does not automatically adjust exposure.') +
      qa('How do I use Peak Inspector?', 'Move over the graph and click a detectable peak to lock the inspector. It can show wavelength, pixel, intensity, FWHM, SNR, an active element match, diffraction-artifact context and quality flags when those values exist. Use Left/Right arrows while locked to move between available peaks, and click again to release.') +
      qa('Why can reference comparison and radial velocity show different shifts?', 'Reference-spectrum alignment is a visual/statistical comparison tool that may normalize and shift a reference to improve overlap. ASTRO radial velocity is separately estimated from compatible measured absorption-line shifts with uncertainty weighting. Comparison alignment is not a radial-velocity measurement.') +
