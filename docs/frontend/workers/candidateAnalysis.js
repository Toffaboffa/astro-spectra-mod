(function (root) {
  ...
  const presetResolver = root.SPECTRA_PRO_presetResolver;
  const clamp = spectrumMath && spectrumMath.clamp;
  const median = spectrumMath && spectrumMath.median;
  const estimateOffset = spectrumMath && spectrumMath.matchOffsetNm;
  const getObservedRange = spectrumMath && spectrumMath.observedRange;
  const guessObservedResolutionNm = spectrumMath && spectrumMath.observedResolutionNm;
  ...
  const getElementFamily = presetResolver && presetResolver.elementFamily;
  const getPresetConfig = presetResolver && presetResolver.resolve;
  const filterAtomicLines = presetResolver && presetResolver.filterAtomicLines;
  const filterMolecularBands = presetResolver && presetResolver.filterMolecularBands;
  ...
  function analyzeFrame(frame, state, options) {
    ...
    const I = Array.isArray(frame.processedI) && frame.processedI.length === frame.I.length
      ? frame.processedI
      : frame.I;
    ...
    const autoTuneSupported = ['smart-gastube', 'smart-atomic', 'smart-molecular', 'lamp-hg'].indexOf(presetCfg.id) !== -1;
    const autoTuneEnabled = autoTuneSupported && opt.autoTune !== false && opt.smartFindEnabled !== false;
    ...
    // Auto tune starts from a permissive master peak set. The fingerprint
    // evidence layer re-evaluates stricter peak/tolerance subsets, so the
    // identification no longer hinges on one manually chosen threshold.
    const peakThresholdRel = autoTuneEnabled ? 0.015 : requestedPeakThresholdRel;
    const peakDistancePx = autoTuneEnabled ? 2 : requestedPeakDistancePx;
    const hardMaxDistanceNm = autoTuneEnabled ? 1.8 : requestedMaxDistanceNm;
    ...
    let peaks = inferPeaks(frame, peakScoring.scorePeaks(rawPeaks, {
    ...
    const features = spectralFeatures.measureFeatures(frame, peaks, {
      polarity: 'emission',
      smoothingRadiusPx: 1,
      windowRadiusPx: Math.max(6, peakDistancePx * 2)
    });
    ...
    peaks = peaks.map(function (peak) {
      const feature = featureByIndex[String(peak.index)];
      if (!feature) return peak;
      return Object.assign({}, peak, {
        nm: Number.isFinite(Number(feature.centerNm)) ? Number(feature.centerNm) : peak.nm,
        featureCenterNm: feature.centerNm,
        centerUncertaintyNm: feature.centerUncertaintyNm,
        fwhmNm: feature.fwhmNm,
        equivalentWidthNm: feature.equivalentWidthNm,
        featureQualityFlags: feature.qualityFlags.slice()
      });
    });

    const calibrationSummary = calibrationDiagnostics.evaluate(opt.calibration || frame.calibration || null, frame);
    const matchUncertaintyModel = calibrationDiagnostics.createMatchingModel(
      calibrationSummary,
      opt.hardware || frame.hardware || null,
      features,
      hardMaxDistanceNm,
      presetCfg.toleranceNm
    );
    ...
    return {
      ...
      maxDistanceNm: effectiveMatchToleranceNm,
      hardMatchCapNm: hardMaxDistanceNm,
      preprocessing: frame.preprocessing && typeof frame.preprocessing === 'object' ? frame.preprocessing : null,
      calibrationDiagnostics: calibrationSummary,
      matchUncertaintyModel: matchUncertaintyModel,
  ...
  root.SPECTRA_PRO_candidateAnalysis = { analyzeFrame: analyzeFrame };
