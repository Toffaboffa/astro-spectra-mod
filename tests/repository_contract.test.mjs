...
assert.ok(spec.includes('Current UI version: **3.1.6**'));
assert.ok(readme.includes('Current UI version: v3.1.6'));
...
for (const relative of releaseSources) {
  assert.ok(read(relative).includes('3.1.6'), relative + ' must carry the v3.1.6 release version');
}
const spectraproPage = read('docs/frontend/pages/spectrapro.html');
assert.ok(spectraproPage.includes('?v=3.1.6'), 'canonical application page must publish 3.1.6 cache keys');
assert.ok(spectraproPage.includes('calibrationScript.js?v=3.1.6'), 'canonical calibration engine must use the v3.1.6 release cache key');
...
for (const relative of lateUiVersionSources) {
  ...
  assert.ok(source.includes('3.1.6'), relative + ' must carry the v3.1.6 app version');
...
assert.ok(uiTweaks.includes("const VERSION = 'v3.1.6';"), 'late UI tweaks must publish the v3.1.6 badge');
...
assert.ok(aiWorker.includes("appVersion: '3.1.6'"), 'AI Worker responses must expose application release 3.1.6');
