...
assert.ok(exportUi.includes("if (Array.isArray(a.topHits)) return a.topHits.slice(0, 80);"), 'deterministic non-Fluorescent PDF tables must use accepted topHits');
assert.ok(!exportUi.includes("if (Array.isArray(a.rawTopHits) && a.rawTopHits.length) return a.rawTopHits.slice(0, 80);"), 'rawTopHits must remain diagnostic data rather than the PDF result-table source');
