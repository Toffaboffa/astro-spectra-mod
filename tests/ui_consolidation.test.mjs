...
assert.ok(spectrapro.includes('graphScript.js?v=3.1.0'), 'published graph code must use a fresh cache key');
assert.ok(spectrapro.includes('stateStore.js?v=3.1.0'), 'dynamic ASTRO label UI loader must use a fresh cache key');
assert.ok(spectrapro.includes('uiPanels.js?v=3.1.0'), 'version badge UI must use a release cache key');
assert.ok(spectrapro.includes('styles.css?v=3.1.0'), 'published Solar preview CSS must use a fresh cache key');
assert.ok(spectrapro.includes('overlays.js?v=3.1.0'), 'published ASTRO overlay must use a fresh cache key');
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.0'), 'published ASTRO controls must use a fresh cache key');
...
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.0'), 'published bootstrap must use a cache key that includes the LAB init fix');
assert.ok(spectrapro.includes('mod-panels.css?v=3.1.0'), 'published ASTRO control styles must use a fresh cache key');
...
assert.ok(spectrapro.includes('imageLoadingScript.js?v=3.1.0'), 'published external-image loader must use a fresh cache key');
...
assert.ok(spectrapro.includes('cameraScript.js?v=3.1.0'), 'published camera controller must use the Auto Pause cache key');
assert.ok(spectrapro.includes('imageLoadingScript.js?v=3.1.0'), 'published image loader must use the stable source-control cache key');
...
assert.ok(spectrapro.includes('calibrationScript.js?v=3.1.0'), 'canonical calibration engine must be cache-versioned');
assert.ok(spectrapro.includes('calibrationPointManager.js?v=3.1.0'), 'CALIBRATE point manager must be cache-versioned');
assert.ok(spectrapro.includes('stateStore.js?v=3.1.0'), 'published state store must use the 3.1.0 cache key');
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.0'), 'published PRO synchronization must use the 3.1.0 cache key');
