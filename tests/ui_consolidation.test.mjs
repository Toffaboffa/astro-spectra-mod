...
for (const id of ['spGraphXAxisMode', 'spGraphYAxisMode', 'spGraphFillMode', 'spGraphPeaks', 'spGraphAnalyze']) {
  assert.equal(markupCount(id), 1, id + ' persistent graph control markup must remain unique');
}
assert.ok(bootstrap.includes("graphXAxisSel && graphXAxisSel.addEventListener('change'"), 'persistent X-axis must proxy the CORE X-axis control');
assert.ok(bootstrap.includes("graphYAxisSel && graphYAxisSel.addEventListener('change'"), 'persistent Y-axis must proxy the CORE Y-axis control');
assert.ok(bootstrap.includes("graphFillModeSel && graphFillModeSel.addEventListener('change'"), 'persistent Fill mode must proxy the CORE Fill mode control');
assert.ok(bootstrap.includes("graphPeaksInput && graphPeaksInput.addEventListener('change'"), 'persistent Peaks must proxy the existing peak toggle');
assert.ok(bootstrap.includes("const target = currentMode === 'ASTRO' ? $('spAstroEnabled') : $('spLabEnabled');"), 'persistent Analyze must proxy the active analysis workspace control');
assert.ok(bootstrap.includes("graphAnalyzeInput.disabled = (analysisMode !== 'LAB' && analysisMode !== 'ASTRO')"), 'persistent Analyze must be disabled in non-analysis workspaces');
...
assert.ok(styles.includes('#SpectraProDockHost #spGraphFillMode{width:98px;}'), 'persistent Fill mode select must fit SYNTHETIC without clipping');
...
assert.ok(spectrapro.includes('graphScript.js?v=3.1.3'), 'published graph code must use a fresh cache key');
assert.ok(spectrapro.includes('stateStore.js?v=3.1.3'), 'dynamic ASTRO label UI loader must use a fresh cache key');
assert.ok(spectrapro.includes('uiPanels.js?v=3.1.3'), 'version badge UI must use a release cache key');
assert.ok(spectrapro.includes('styles.css?v=3.1.3'), 'published Solar preview CSS must use a fresh cache key');
assert.ok(spectrapro.includes('overlays.js?v=3.1.3'), 'published ASTRO overlay must use a fresh cache key');
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.3'), 'published ASTRO controls must use a fresh cache key');
...
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.3'), 'published bootstrap must use a cache key that includes the LAB init fix');
assert.ok(spectrapro.includes('mod-panels.css?v=3.1.3'), 'published ASTRO control styles must use a fresh cache key');
...
assert.ok(spectrapro.includes('imageLoadingScript.js?v=3.1.3'), 'published external-image loader must use a fresh cache key');
...
assert.ok(spectrapro.includes('cameraScript.js?v=3.1.3'), 'published camera controller must use the Auto Pause cache key');
assert.ok(spectrapro.includes('imageLoadingScript.js?v=3.1.3'), 'published image loader must use the stable source-control cache key');
...
assert.ok(spectrapro.includes('calibrationScript.js?v=3.1.3'), 'canonical calibration engine must be cache-versioned');
assert.ok(spectrapro.includes('calibrationPointManager.js?v=3.1.3'), 'CALIBRATE point manager must be cache-versioned');
assert.ok(spectrapro.includes('stateStore.js?v=3.1.3'), 'published state store must use the 3.1.3 cache key');
assert.ok(spectrapro.includes('proBootstrap.js?v=3.1.3'), 'published PRO synchronization must use the 3.1.3 cache key');
