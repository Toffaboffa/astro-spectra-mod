...
assert.ok(spectrapro.includes('analysisWorkerClient.js?v=3.1.6-offset-1'), 'published worker client must use the wavelength-offset cache key');
...
assert.ok(workerClient.includes("workerUrl: '../workers/analysis.worker.js?v=3.1.6-offset-1'"), 'worker client must load the refreshed wavelength-offset worker shell');
assert.ok(stateStore.includes("const AI_ASSET_VERSION = '3.1.6-offset-1';"), 'dynamic export/AI modules must use the refreshed wavelength-offset cache key');
assert.ok(workerClient.includes('analysisNext.rawMatchOffsetNm = Number.isFinite(Number(msg.payload.rawMatchOffsetNm))'), 'worker results must preserve the broader matcher offset separately');
assert.ok(workerClient.includes("analysisNext.offsetBasis = msg.payload.offsetBasis ? String(msg.payload.offsetBasis) : 'matcher-residuals';"), 'worker results must persist wavelength-offset provenance');
