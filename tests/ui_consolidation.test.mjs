...
assert.ok(spectrapro.includes('analysisWorkerClient.js?v=3.1.6-snr-1'), 'published worker client must use the canonical-SNR cache key');
assert.ok(spectrapro.includes('dataQualityPanel.js?v=3.1.6-snr-1'), 'published Data Quality module must use the canonical-SNR cache key');
assert.ok(workerClient.includes("workerUrl: '../workers/analysis.worker.js?v=3.1.6-snr-1'"), 'worker client must load the refreshed canonical-SNR worker shell');
assert.ok(stateStore.includes("const AI_ASSET_VERSION = '3.1.6-cal-diag-1';"), 'dynamic export/AI modules must use the corrected calibration-diagnostics cache key');
