...
- The language choice is not persisted across reloads in v3.1.5; English is always the clean baseline after a new load.
...
SPECTRA PRO v3.1.5 uses the visually buried CORE export entry point with a global **EXPORT** control beside the Dark/Ref capture controls in the left source panel. The Dark/Ref buttons are arranged as two centered, symmetric rows and the larger export button remains available regardless of the active PRO workspace.
